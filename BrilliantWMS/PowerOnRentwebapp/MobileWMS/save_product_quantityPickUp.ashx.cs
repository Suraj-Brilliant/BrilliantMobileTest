...
{
    /// <summary>
    /// Summary description for save_product_quantityPickUp
    /// </summary>
    public class save_product_quantityPickUp : IHttpHandler
    {
        ...
        long WarehouseID = 0, orderID = 0, UserID = 0, CustomerID = 0, CompanyID = 0, LocID = 0, ProductID = 0;
        decimal Qty = 0;

        string objname = "", page = "", finalzone = "", serialno = "";
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (context.Request.QueryString["serial"] != null)
            {
                serialno =(context.Request.QueryString["serial"]).ToString();
            }
            ...
            if (PKID > 0)
            {
                ...
                decimal pickupqty = ProdPickUpQty(PKID, ProductID,LocID);
                decimal RemQty = orderqty - pickupqty;
                if (Qty - pickupqty > RemQty)
                {
                ...
                {
                    decimal SkuStock = SkuLocStock(ProductID, LocID);
                    if(RemQty> SkuStock)
                    {
                        jsonString = jsonString + "\"status\": \"failed\",\n";
                        jsonString = jsonString + "\"reason\": \"SKU Stock Not Available In Location\"\n";
                    }
                    else
                    {
                        int result = UpdatePickUpQty(PKID, ProductID, Qty, LocID, objname);
                        ...
                        if(count==0)
                        {
                            UpdateQCQty(PKID, ProductID, Qty, LocID, objname);
                        }
                        if (result > 0)
                        {
        ...
        public int UpdateQCQty(long pkid, long prodid, decimal qty, long locid, string obj)
        {
            int result = 0;
            try
            {
                ...
                cmdDetail.CommandText = "SP_UpdateQCQty";
            ...
            { Login.Profile.ErrorHandling(ex, "save_product_quantityPickUp", "UpdateQCQty"); }
            finally
            { }
            return result;
        }
        ...
        public decimal SkuLocStock(long prdid, long locid)
        {
            ...
            try
            {
                ...
                cmd1.CommandText = "SP_SkuLocStock";
                ...
                if (dt1.Rows.Count > 0)
                {
                    orderqty = Convert.ToDecimal(dt1.Rows[0]["SkuStock"]);
                }
            }
            catch (Exception ex)
        ...
        public int UpdatePickUpQty(long grnid, long prodid, decimal qty, long locid, string obj)
        {
            int result = 0;
            try
            {
                ...
                SqlConnection conn = new SqlConnection(strcon);
                cmdDetail.CommandType = CommandType.StoredProcedure;
                cmdDetail.CommandText = "SP_UpdatePickUpQty";
                cmdDetail.Connection = conn;
                cmdDetail.Parameters.Clear();
        ...
        public decimal ProdPickUpQty(long pkid, long prdid,long locid)
        {
            SqlCommand cmd1 = new SqlCommand();
            SqlDataAdapter da1 = new SqlDataAdapter();
            DataSet ds1 = new DataSet();
            ...
            try
            {
                SqlConnection conn = new SqlConnection(strcon);
                cmd1.CommandType = CommandType.StoredProcedure;
                cmd1.CommandText = "SP_ProdPickUpQty";
                cmd1.Connection = conn;
                cmd1.Parameters.Clear();
                cmd1.Parameters.AddWithValue("@ptid", pkid);
                cmd1.Parameters.AddWithValue("@prdid", prdid);
                cmd1.Parameters.AddWithValue("@locid", locid);
                da1.SelectCommand = cmd1;
                da1.Fill(ds1, "tbl1");
                ...
                if (dt1.Rows.Count > 0)
                {
                    orderqty = Convert.ToDecimal(dt1.Rows[0]["PickUpQty"]);
                }
            }
            catch (Exception ex)
            { Login.Profile.ErrorHandling(ex, "save_product_quantityPutIn", "ProdPutInQty"); }
        ...
        public decimal ProdOrderQty(long oid, long prdid, string page,string obj)
        {
            ...
            { Login.Profile.ErrorHandling(ex, "save_product_quantityPickUp", "ProdOrderQty"); }
            finally
            { }
            return orderqty;
        }
        ...
        public long gettbltype(string tbltype)
        {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.CommandText = "sp_checkPickUpProductRecord";
            cmd.Connection = conn;
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@obj", objname);
            cmd.Parameters.AddWithValue("@picupid", orderID);
            cmd.Parameters.AddWithValue("@pkid", PKID);
            cmd.Parameters.AddWithValue("@prdid", ProductID);
            cmd.Parameters.AddWithValue("@locid", LocID);
            cmd.Parameters.AddWithValue("@qty", Qty);
        ...
        public long getStatus(string objNM)
        {
            ...
            try
            {
                ...
                cmd2.CommandText = "select * from mstatus where Remark='" + objNM + "' and status like '%Transfer PickUP%' ";
        ...
        public int SavePickUpDetail(long oid, string obj, long pickupid, long prodid, decimal qty, long locid, string srno)
        {
            int result = 0;
            try
            {
