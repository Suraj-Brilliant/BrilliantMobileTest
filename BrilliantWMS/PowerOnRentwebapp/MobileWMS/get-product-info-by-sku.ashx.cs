...
{
    /// <summary>
    /// Summary description for get_product_info_by_sku
    /// </summary>
    public class get_product_info_by_sku : IHttpHandler
    {
        ...
        long WarehouseID = 0; string skuid = "";
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            skuid = context.Request.QueryString["sku"];

            if (context.Request.QueryString["sku"] != null)
            {
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "Select V.ID,V.Name,V.ProductCode,V.Description,V.BatchCode,V.LocationID,V.LocationCode,V.ClosingBalance,V.Path,V.SkuImage,V.WarehouseID,W.WarehouseName,P.OMSSKUCode  from V_WMS_SKUSearch V left outer join mWarehouseMaster W on V.WarehouseID =W.ID  left outer join mProduct P on V.ID=P.ID  where V.WarehouseID = " + WarehouseID + " and P.OMSSKUCode='" +skuid + "' ";
                cmd.Connection = conn;
