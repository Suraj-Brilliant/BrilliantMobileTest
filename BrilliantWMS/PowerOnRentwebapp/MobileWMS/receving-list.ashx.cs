...
{
    /// <summary>
    /// Summary description for receving_list
    /// </summary>
    public class receving_list : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (context.Request.QueryString["uid"] != null)
            {
                ...
                //cmd.CommandText = "SELECT  T.TaskID, T.ObjectName, T.AssignTo, T.TaskDate, T.TaskECD, case when T.TaskRemark='' then 'N/A' else T.TaskRemark end TaskRemark, T.CreatedBy, U.FirstName+' '+U.LastName AssignBy, T.CreationDate, T.Active, T.Priority, T.JobCardName,J.OID QCID,Q.Status,S.Status StatusName,Case When Q.Status<=33 then 'Pending'  when Q.Status=35 then 'Completed' end PutInStatus FROM  tTaskDetail T left outer join tJobCardDetail J on T.TaskID=J.TaskID left outer join tQualityControlHead Q on J.OID=Q.ID left outer join mStatus S on Q.Status=S.ID left outer join mUserProfilehead U on T.CreatedBy=U.ID where   T.ObjectName=10003 and T.AssignTo=" + UserID + " and Q.Status!=35";
                if(skey!="")
                {
                    cmd.CommandText = "exec PutInListMobile1  " + UserID + "," + WarehouseID + "," + skey + "";
                }
                else
                {
                    cmd.CommandText = "exec PutInListMobile  " + UserID + "," + WarehouseID + "";
                }
                ...
                if (cntr > 0)
                {
                    ...
                    {
                        string id = dt.Rows[i]["QCID"].ToString();
                        ...
                        string status = dt.Rows[i]["PutInStatus"].ToString();
