...
$(document).ready(function () {
    var Val = document.getElementById("usrID").value;
    GetDropDownData('71','#ddlCustomer', Val, '0');
});
...
function GetDropDownData(CtlID, FilterCrtl, FilterId, selectedID) {
    ...
    $.ajax({
        ...
        url: "../WMS/ReportWMS.aspx/GetFilterData",
        data: params,
        dataType: "json",
...
function selectedRec() {
    ...
    if (GvList.Rows.length > 0) {
       ...
       if (hdnListVal.value != null && hdnListVal.value != "" && hdnListVal.value != "undefined") {
            ...
            window.open("../WMS/RptViewer.aspx?Val=" + hdnListVal.value + "&fromDt=" + fromDt + "&ToDt=" + ToDt + "&RptType=" + "M" + "&RptID=" + ReportId, 'liveMatches', 'directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=800,height=800');
    ...
    if (GvList.Rows.length > 0) {
     ...
     //   if (hdnListVal.value != null && hdnListVal.value != "" && hdnListVal.value == "undefined") {
        if (hdnListVal.value != null && hdnListVal.value != "" && hdnListVal.value != "undefined") {
            ...
            window.open("../WMS/RptViewer.aspx?Val=" + hdnListVal.value + "&fromDt=" + fromDt + "&ToDt=" + ToDt + "&RptType=" + "S" + "&RptID=" + ReportId, 'liveMatches', 'directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=500,height=500');
