...
var s_d = new Array();
s_d[0] = "";
s_d[21] = "Akola|Amravati|Aurangabad|Bhandara|Bid|Buldhana|Chandrapur|Osmanabad|Dhule|Gadchiroli|Gondia|Ahmadnagar|Mumbai|Hingoli|Jalgaon|Jalna|Kolhapur|Latur|Nagpur|Nanded|Nandurbar|Nashik|Parbhani|Pune|Raigad|Ratnagiri|Sangli|Satara|Sindhudurg|Solapur|Thane|Wardha|Washim|Yavatmal";

var s_t = new Array();
s_t[0] = "";
s_t[24] = "Ambegaon|Baramati|Bhor|Daund|Haveli|Indapur|Junnar|Khed|Lonavala|Maval|Mulshi|Malegaon|Pune City|Purandar|Shirur|Velhe"; //Pune
s_t[25] = "Alibag|Khalapur|Karjat|Murud|Mangaov|Mahad|Mhasala|Panvel|Pen|Poladpur|Roha|Shrivardhan|Sudhagad Pali|Tala|Uran"; //Raigad

function print_country(country_id) {
    // given the id of the <select> tag as function argument, it inserts <option> tags
    var option_strcountry = document.getElementById(country_id);
    option_strcountry.length = 0;

    var option0 = document.createElement("option");
    option0.text = 'Select Country';
    option0.value = '0';

    try {
        option_strcountry.add(option0, null); //Standard 
    } catch (error) {
        option_strcountry.add(option0); // IE only
    }
    ...
    for (var i = 0; i < country_arr.length; i++) {
        ...
        try {
            option_strcountry.add(option, null); //Standard 
        } catch (error) {
            option_strcountry.add(option); // IE only
        }
    }
    option_strcountry.selectedIndex = 0;
}

function print_state(state_id, state_index) {
    var option_strstate = document.getElementById(state_id);
    option_strstate.length = 0; // Fixed by Julian Woods
    var option0 = document.createElement("option");
    option0.text = 'Select State';
    option0.value = '0';

    try {
        option_strstate.add(option0, null); //Standard 
    } catch (error) {
        option_strstate.add(option0); // IE only
    }

    var state_arr = s_a[state_index].split("|");
    for (var i = 0; i < state_arr.length; i++) {
        var option = document.createElement("option");
        option.text = state_arr[i];
        option.value = state_arr[i];

        try {
            option_strstate.add(option, null); //Standard 
        } catch (error) {
            option_strstate.add(option); // IE only
        }
    }
    option_strstate.selectedIndex = 0;
}
