...
{
    public partial class ApproverList : System.Web.UI.Page
    {
        ResourceManager rm;
        CultureInfo ci;
        long StoreId = 0;
        ...
        DataSet GetPrdLst(int pageIndex, string filter)
        {
            ...
            if (filter == "")
            {
               // str = "select Id,ProductCode,OMSSKUCode,Name,Description from mProduct where StoreId = '" + StoreId + "'";
                if (StoreId != 0)
                {
                    str = "select uh.ID, uh.FirstName+ ' '+ uh.LastName AName,uh.EmailID,t.Territory,c.Name from mUserProfileHead uh left outer join mTerritory t on uh.DepartmentID = t.ID left outer join mCompany c on uh.CompanyID = c.ID where uh.DepartmentID = '" + StoreId + "' and  uh.UserType in ('Admin','Requestor And Approver','Approver')";
                }
                else
                {
                    str = "select uh.ID, uh.FirstName+ ' '+ uh.LastName AName,uh.EmailID,t.Territory,c.Name from mUserProfileHead uh left outer join mTerritory t on uh.DepartmentID = t.ID left outer join mCompany c on uh.CompanyID = c.ID where uh.CompanyID ='" + companyID + "' uh.UserType in ('Admin','Requestor And Approver','Approver')";
                }
            ...
            else
            {
                if (StoreId != 0)
                {
                    str = "select uh.ID, uh.FirstName+ ' '+ uh.LastName AName,uh.EmailID,t.Territory,c.Name from mUserProfileHead uh left outer join mTerritory t on uh.DepartmentID = t.ID left outer join mCompany c on uh.CompanyID = c.ID where uh.DepartmentID = '" + StoreId + "' and  uh.UserType in ('Admin','Requestor And Approver','Approver') and (uh.FirstName like '%" + filter + "%' or t.Territory like '%" + filter + "%' or c.Name like '%" + filter + "%')";
                }
                else
                {
                    str = "select uh.ID, uh.FirstName+ ' '+ uh.LastName AName,uh.EmailID,t.Territory,c.Name from mUserProfileHead uh left outer join mTerritory t on uh.DepartmentID = t.ID left outer join mCompany c on uh.CompanyID = c.ID where uh.UserType in ('Admin','Requestor And Approver','Approver') and (uh.FirstName like '%" + filter + "%' or t.Territory like '%" + filter + "%' or c.Name like '%" + filter + "%')";
                }
                SqlDataAdapter da = new System.Data.SqlClient.SqlDataAdapter(str, conn);
                ds1.Reset();
                da.Fill(ds1);
        ...
        private void loadstring()
        {
            ...
            {
                Login.Profile.ErrorHandling(ex, this, "ApprovalList", "loadstring");
            }
        }
