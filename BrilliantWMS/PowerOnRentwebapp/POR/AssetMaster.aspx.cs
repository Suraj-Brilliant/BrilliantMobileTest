    ...
    public partial class AssetMaster : System.Web.UI.Page
    {
        ...
        public static long PMSaveNewRates(object newPrice0)
        {
            ...
            try
            {
                ...
                if (dictionary["ProdID"].ToString() != "0" && dictionary["ProdID"].ToString() != "")
                {
                    ...
                    if (dictionary["EffectiveDate"] != null) newSite.EffectiveDate = Convert.ToDateTime(dictionary["EffectiveDate"]);
                    if (dictionary["StartDate"] != null) newSite.StartDate = Convert.ToDateTime(dictionary["StartDate"]);
                    if (dictionary["EndDate"] != null) newSite.Enddate = Convert.ToDateTime(dictionary["EndDate"]);
        ...
        protected void GVRateHistory()
        {
            ...
            try
            {
                ...
                if (hdnprodID.Value != "" && hdnprodID.Value != "0")
                {
                    GVRateHistory1.DataSource = productClient.GetToolSiteHistory(Convert.ToInt64(hdnprodID.Value), profile.DBConnection._constr);
                    GVRateHistory1.DataBind();
                }
            }
